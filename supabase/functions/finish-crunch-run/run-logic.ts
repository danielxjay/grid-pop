  ...
  for (let row = 0; row < GRID_SIZE; row += 1) {
    ...
    if (advance.advanceRight && rightNewCol >= leftNewCol && (!advance.advanceLeft || rightNewCol > leftNewCol)) {
      ...
      if (chain.length === 0 && atRightNew !== null && !isCrunchWall(atRightNew)) {
        board[toIndex(row, rightNewCol)] = null;
      }
...
export function replayCrunchRun(
  ...
  for (const move of moves) {
    ...
    if (nextGame === advanced.game) {
      ...
      const piece = advanced.game.tray.find((entry) => entry?.id === move.pieceId) ?? null;
      const trayIds = advanced.game.tray.map((entry) => entry?.id ?? null);
      let reason = "unknown";
      if (!piece) {
        reason = `piece ${move.pieceId} not in tray [${trayIds.join(",")}]`;
      } else {
        const bounds = piece.bounds;
        const occupiedCells = piece.shape.cells
          .map((cell_offset: readonly [number, number]) => {
            const [dx, dy] = cell_offset;
            const idx = toIndex(move.row + dy, move.col + dx);
            const cell = advanced.game.board[idx];
            return cell ? `(${move.row + dy},${move.col + dx})=${JSON.stringify(cell?.groupId)}` : null;
          })
          .filter(Boolean);
        reason = `canPlace failed at (${move.row},${move.col}) shape=${piece.shape.name} oob=${move.row < 0 || move.col < 0 || move.row + bounds.height > GRID_SIZE || move.col + bounds.width > GRID_SIZE} occupied=[${occupiedCells.join(",")}]`;
      }
      console.error(`[crunch-invalid] move#${moveIndex} pieceId=${move.pieceId} row=${move.row} col=${move.col} atMs=${move.atMs} wallDepth=${advanced.game.crunchWallDepth} reason=${reason}`);
