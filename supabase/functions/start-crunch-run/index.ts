...
import { createCrunchGameState } from "./run-logic.ts";
...
Deno.serve(async (req) => {
  try {
    ...
    if (activeRunsError) {
      return json({ error: "Could not check your existing Crunch game." }, { status: 500 });
    }

    const existingRuns = (activeRuns ?? []) as ActiveCrunchRunRow[];
    ...
    if (runIdsToAbandon.length > 0) {
      ...
      if (abandonError) {
        return json({ error: "Could not prepare a new Crunch game right now." }, { status: 500 });
      }
    }
    ...
    const traySeed = crypto.randomUUID();
    const wallSeed = crypto.randomUUID();
    const revealSecret = crypto.randomUUID();
    const game = createCrunchGameState(traySeed, wallSeed);
    ...
    if (runError || !run) {
      return json({ error: "Could not start Crunch right now." }, { status: 500 });
    }
    ...
    return json({
      ...
      nextWaveAtMs: game.nextWaveAtMs,
      wallRngState: game.wallRngState,
      trayRngState: game.trayRngState,
    });
  } catch (error) {
    console.error("Unexpected start-crunch-run failure.", error);
    return json({ error: "Unexpected start-crunch-run failure." }, { status: 500 });
  }
});
