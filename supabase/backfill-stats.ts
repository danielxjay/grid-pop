/**
 * Backfill best_combo and best_move_score on the scores table for all verified runs.
 *
 * Usage:
 *   SUPABASE_URL=... SUPABASE_SERVICE_ROLE_KEY=... deno run --allow-env --allow-net backfill-stats.ts
 *
 * Safe to run multiple times — only processes rows where best_combo IS NULL.
 */
...
function getShapeBounds(shape: (typeof SHAPES)[number]) {
  return {
    width: Math.max(...shape.cells.map(([dx]) => dx)) + 1,
    height: Math.max(...shape.cells.map(([, dy]) => dy)) + 1,
  };
}
...
function buildTray(board: BoardCell[], nextPieceId: number, rngState: number) {
  ...
  for (let slot = 0; slot < TRAY_SIZE; slot++) {
    ...
    tray.push(p.piece);
    currentPieceId = p.nextPieceId;
    currentRngState = p.rngState;
  }
...
function findClears(board: BoardCell[]) {
  ...
  for (let row = 0; row < GRID_SIZE; row++) {
    const indices = Array.from({ length: GRID_SIZE }, (_, col) => toIndex(row, col));
    if (indices.every((i) => board[i] !== null)) indices.forEach((i) => cleared.add(i));
  }
  for (let col = 0; col < GRID_SIZE; col++) {
    const indices = Array.from({ length: GRID_SIZE }, (_, row) => toIndex(row, col));
    if (indices.every((i) => board[i] !== null)) indices.forEach((i) => cleared.add(i));
  }
  return cleared;
}
...
function countLines(clearedIndices: Set<number>) {
  ...
  for (let row = 0; row < GRID_SIZE; row++) {
    const indices = Array.from({ length: GRID_SIZE }, (_, col) => toIndex(row, col));
    if (indices.every((i) => clearedIndices.has(i))) lines++;
  }
  for (let col = 0; col < GRID_SIZE; col++) {
    const indices = Array.from({ length: GRID_SIZE }, (_, row) => toIndex(row, col));
    if (indices.every((i) => clearedIndices.has(i))) lines++;
...
function computeRunStats(seed: string, moves: Move[]) {
  ...
  for (const move of moves) {
    ...
    // Peek at lines cleared for this placement before applying it
...
console.log("Fetching verified runs with unprocessed stats...");
...
console.log(`Processing ${rows.length} run(s)...`);
...
for (const row of rows) {
  ...
  if (!stats) {
    console.warn(`  Skipping score ${row.id} — replay failed.`);
    failed++;
    continue;
  }
  ...
  if (updateError) {
    console.warn(`  Failed to update score ${row.id}: ${updateError.message}`);
    failed++;
  } else {
    console.log(`  Updated score ${row.id} — combo: ${stats.bestCombo}, best move: ${stats.bestMoveScore}, best lines: ${stats.bestLinesCleared}, moves: ${stats.moveCount}`);
    succeeded++;
  }
}
